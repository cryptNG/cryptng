  ...
  //gettransactionreceipt  -> contractAddress
   ...
   if (window.ethereum) {
    ...
    let lweb3 = new Web3('http://192.168.0.7:8545');
            ...
            const lastBlockHex = await lweb3.eth.getBlockNumber();
            let lastBlockDec = web3.utils.hexToNumber(lastBlockHex);


            while(lastBlockDec > 0)
            {
              lastBlockDec--;
              const block = await lweb3.eth.getBlock(web3.utils.numberToHex(lastBlockDec), false);
