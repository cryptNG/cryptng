 ...
 * {ERC721Enumerable}.
 */
contract BasicProofingToken is
    Context,
    ERC165,
    IERC721,
    IERC721Metadata,
    Ownable
{
    ...
    mapping(uint256 => string) _tokenMessage;
    uint256[] _tokens;
    uint32[] _types;
    ...
    event MintedHashMapProof(address creator ,uint256 fromHash, uint256 toHash, uint256 tokenId);
    ...
    function mintHashMapProof(uint256 fromHash, uint256 toHash) public isService()
    {
        require(_hashMap[fromHash] == 0, "You cannot overwrite existing proof!");
        require(toHash != 0 && fromHash != 0, "You cannot proof NULL (0) values!");
        _hashMap[fromHash] = toHash;
       

        // tokenID + type * _maxTokens
        uint256 typedTokenId = (_tokens.length ) + 1 * _maxTokens; 

        _mint(_msgSender(), typedTokenId);

        emit MintedHashMapProof(_msgSender(), fromHash, toHash, typedTokenId);
    }

    //  function mintBidirectionalHashMapProof(uint256 fromHash, uint256 toHash) public
    // {
    //     mintHashMapProof(fromHash, toHash);
    //     mintHashMapProof(toHash, fromHash);
    // }
    ...
    function proofHashMap(uint256 fromHash) public view returns (uint256)
    {
        require(_hashMap[fromHash] != 0, "The proof you are requesting does not exist!");
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = BasicProofingToken.ownerOf(tokenId);
    ...
    function _transfer(
        ...
        require(
            BasicProofingToken.ownerOf(tokenId) == from,
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(BasicProofingToken.ownerOf(tokenId), to, tokenId);
    }
