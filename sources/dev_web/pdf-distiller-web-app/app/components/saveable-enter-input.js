...
export default class SaveableEnterInput extends Component {
  ...
  loadFromSessionIfSaved() {
    if (
      sessionStorage.getItem('distiller_hassaved_' + this.contentType) == 'true'
    ) {
      console.log(
        'session save is active for ' + 'distiller_hassaved_' + this.contentType
      );
      this.inputContent = sessionStorage.getItem(
        'distiller_' + this.contentType
      );
